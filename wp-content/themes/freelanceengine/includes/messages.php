...
class Fre_MessageAction extends AE_Base {
	...
	function sendMessage() {
		...
		if ( isset( $_REQUEST['miilestone_type'] ) ) {
			$miilestone_type = $_REQUEST['miilestone_type'];

			$milestone_comment = $_REQUEST;
			$milestone_comment['comment_content'] = 'Requested documents';
			$milestone_comment['comment_approved'] = 1;

			$m_comment = $this->comment->insert( $milestone_comment );
			update_comment_meta( $m_comment, 'changed_milestone_id', 0 );
		}
