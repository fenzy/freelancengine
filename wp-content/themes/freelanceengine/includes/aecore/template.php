...
if ( ! function_exists( 'ae_project_category_list' ) ) {
	/**
	 * Display or retrieve the HTML list of categories
	 */
	function ae_project_category_list( $tax ) {
		...
		$output = '<div class="job_branch">';
		 
		if ( !empty($taxonomies) ) :
		    foreach( $taxonomies as $category ) {
		        if( $category->parent == 0 ) {
		        	$icon_url = esc_attr(get_bloginfo('template_url') . '/img/' . $category->slug . '.png');

		            $output.= '<div class="col-sm-2"><div class="fre-btn category_item" id="'. esc_attr( $category->term_id ) .'" style="background-image: url('. $icon_url . ');"><i class="fa fa-check-circle fa-2x hidden" aria-hidden="true"></i></div><li>' . esc_attr( $category->name ) . '</li></div>';
		        }
		    }
		endif;
