...
class AE_Posts {
	...
	public function insert( $args ) {
		...
		if ( $result != false && ! is_wp_error( $result ) ) {
			...
			// add custom post meta for projects

			if ($args['post_type'] == PROJECT) {
				update_post_meta( $args['id'], 'job_address', $args['job_address'] );
				update_post_meta( $args['id'], 'job_city', $args['job_city'] );
				update_post_meta( $args['id'], 'job_state', $args['job_state'] );
				update_post_meta( $args['id'], 'job_zipcode', $args['job_zipcode'] );
				update_post_meta( $args['id'], 'job_latlng', $args['job_latlng'] );
				update_post_meta( $args['id'], 'job_result', $args['job_result'] );
				update_post_meta( $args['id'], 'job_other_result', $args['job_other_result'] );
			}
