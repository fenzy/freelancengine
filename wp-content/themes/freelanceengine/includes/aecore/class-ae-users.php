...
class AE_Users {
	...
	public function insert( $user_data ) {
		...
		if ( $result != false && ! is_wp_error( $result ) ) {
			...
			if ( isset( $user_data['user_phone_number'] ) ) {
				$description .= '1. Phone number: ' . $user_data['user_phone_number'] . PHP_EOL;
			}

			if ( isset( $user_data['user_address'] ) ) {
				$description .= '2. Address: ' . $user_data['user_address'] . PHP_EOL;
			}

			if ( isset( $user_data['user_age16'] ) ) {
				if ( $user_data['user_age16'] == 1)  {
					$description .= '3. 16 or older? Yes' . PHP_EOL;
				} else {
					$description .= '3. 16 or older? No' . PHP_EOL;
				}
			}

			if ( isset( $user_data['user_student'] ) ) {
				if ( $user_data['user_student'] == 1)  {
					$description .= '4. Student or Not: Student' . PHP_EOL;
				} else {
					$description .= '4. Student or Not: Not Student' . PHP_EOL;
				}
			}

			if ( isset( $user_data['user_education'] ) ) {
				$description .= '5. Education: ' . $user_data['user_education'] . PHP_EOL;
			}

			if ( isset( $user_data['user_skill'] ) ) {
				$description .= '6. Field of Work: ' . $user_data['user_skill'] . PHP_EOL;
			}

			$description .= '7. Experience: ';

			if ( isset( $user_data['user_company_a'] ) ) {
				$description .= $user_data['user_company_a'] . PHP_EOL;
			}

			if ( isset( $user_data['user_company_b'] ) ) {
				$description .= $user_data['user_company_b'] . PHP_EOL;
			}

			if ( isset( $user_data['user_company_c'] ) ) {
				$description .= $user_data['user_company_c'] . PHP_EOL;
			}

			if ( isset( $user_data['user_hear'] ) ) {
				$description .= '8. How did you hear about?: ' . $user_data['user_hear'];
			}

			update_user_meta( $result, 'description', $description );
