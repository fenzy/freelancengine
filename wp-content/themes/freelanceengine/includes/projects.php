...
class Fre_ProjectAction extends AE_PostAction {
	...
	function ae_convert_project( $result ) {
		...
		// get custom field
		$job_address = get_post_meta( $result->ID, 'job_address', true );
		if ($job_address) {
			$result->job_address =  $job_address;
		}

		$job_city = get_post_meta( $result->ID, 'job_city', true );
		if ($job_city) {
			$result->job_city =  $job_city;
		}

		$job_state = get_post_meta( $result->ID, 'job_state', true );
		if ($job_state) {
			$result->job_state =  $job_state;
		}

		$job_zipcode = get_post_meta( $result->ID, 'job_zipcode', true );
		if ($job_zipcode) {
			$result->job_zipcode =  $job_zipcode;
		}

		$job_latlng = get_post_meta( $result->ID, 'job_latlng', true );
		if ($job_latlng) {
			$result->job_latlng =  $job_latlng;
		}

		$job_result = get_post_meta( $result->ID, 'job_result', true );
		if ($job_result) {
			$result->job_result =  $job_result;
		}

		$job_other_result = get_post_meta( $result->ID, 'job_other_result', true );
		if ($job_other_result) {
			$result->job_other_result =  $job_other_result;
		}
