...
/* google shorten url functions */
function google_get_url($url, $shorten = true) {
	// Get API key from : http://code.google.com/apis/console/
	$apiKey = 'AIzaSyBGvEbLnWul3kzeCt4t8Bj4uEbAkCqNrHg';
    ...
    if($shorten) {
        ...
        curl_setopt($curlObj, CURLOPT_URL, $apiURL.'&shortUrl='.$url);
    }
    ...
    if (isset($json->error)) {
        return "error from Goggle:" . $json->error->message;
    }
	return $json->id;
}

function get_bulk_url($url) {
    $curlObj = curl_init();
    
    $fields = array(
        'srcUrls' => urlencode($url),
    ...
    $postFields = "srcUrls=" . $url . "&submit=submit";
    rtrim($fields_string, '&');
    
    curl_setopt($curlObj, CURLOPT_URL, "https://www.webconfs.com/bulk-url-shortener.php");
    ...
    return trim($items->item(0)->nodeValue);
}
...
if ( !class_exists( 'toc' ) ) :
	class toc {
		...
		function __construct()
		{
			...
			$this->toc_post_types = array('page','post');
		...
		function wp_enqueue_scripts()
		{
			...
			wp_register_script( 'toc_mega', $this->path . '/mega-menu.js', array('jquery'), TOC_VERSION, true );
			wp_enqueue_script( 'toc_mega' );
		...
		private function save_admin_options()
		{
			...
			$this->options = array_merge(
				$this->options,
				array(
					'use_google' => (isset($_POST['use_google']) && $_POST['use_google']) ? true : false,
...
<tbody>
<tr>
	<th><label for="use_google">Use Google Shorten Url</label></th>
	<td><input type="checkbox" value="1" id="use_google" name="use_google" <?php if ( $this->options['use_google'] ) echo ' checked="checked"'; ?> /></td>
</tr>
<tr>
...
<tr>
	...
	<td><?php
			foreach ($this->toc_post_types as $post_type) {
...
<?php
		...
		private function build_hierarchy( &$matches )
		{
			...
			$db_options = get_option("toc-options");
			$short_url = "";
            if ($db_options['use_google']) {
				...
				$short_url = get_bulk_url($protocol.$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI']);
			...
			for ($i = 0; $i < count($matches); $i++) {
				...
				if ( in_array($matches[$i][2], $this->options['heading_levels']) ) {
					...
					if ($db_options['use_google'] == 1) {
						$html .= '<a href="' . $short_url . '#' . $this->url_anchor_target( $matches[$i][0]) . '">';
					} else {
		...
		function the_content( $content )
		{
			...
			if ( $this->is_eligible($custom_toc_position) ) {
				...
				if ( $items ) {
					...
					if ( $this->options['show_toc_in_widget_only'] && (in_array($this->toc_post_types, $this->options['show_toc_in_widget_only_post_types'])) ) {
